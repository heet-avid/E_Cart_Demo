$(document).ready(function () {
                ...
                data: 'quantity', className: 'td-number', render: function (data, type, row) {
                    return `<div class="input-group input-group-sm">
                            <div class="btn-group btn-group-sm">
                                <button class="btn btn-round btn-secondary" onclick="updateQuantity(${row.id}, -1)"> <i class="ri-subtract-line"></i> </button>
                                <input type="text" class="form-control text-center quantity mx-1" style="max-width: 50px;" value="${data}" readonly>
                                <button class="btn btn-round btn-secondary" onclick="updateQuantity(${row.id}, 1)"> <i class="ri-add-line"></i> </button>
                            </div>`;
                }
            },
            {
    ...
    function updateTotal() {
        var total = 0;
        $('#cartDT').DataTable().rows().every(function (rowIdx, tableLoop, rowLoop) {
            var data = this.data();
