$(document).ready(function () {
        ...
        columns: [
            ...
            { data: 'status' },
            {
                data: null, sorting:false,className:"td-actions", render: function (data, type, row) {
                    var button = '<button class="btn btn-primary btn-just-icon btn-sm view-order"><i class="ri-eye-line"></i></button>';
                    return button;
                }
            }
        ]
    });
    var orderItemTable = $('#orderItemsDT').DataTable({
        "bPaginate": false,
        "bLengthChange": false,
        "bFilter": false,
        "bInfo": false,
    ...
    window.updateTotal = function () {
        var total = 0;
        $('#orderItemsDT').DataTable().rows().every(function (rowIdx, tableLoop, rowLoop) {
            var data = this.data();
